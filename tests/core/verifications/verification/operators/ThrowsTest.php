...
/**/
	 ...
	 */
	public function testValue2IsArray($value2, $throwException, $expectedResult)
	{
		$this->runInTestCallback($this->createSpecIt(), function($test, $it) use(&$verification, $value2, $throwException){
			$verification = new Verification(function() use($value2, $throwException)
			{
				if ($throwException)
		...
		if ($throwException)
			$this->assertSame($throwException, $verification->getCallDetails()->getValue1()); // Sets thrown exception to value1 instead of default value (function)
		else
