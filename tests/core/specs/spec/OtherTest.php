...
/**/
	...
	public function testIsRoot_ReturnsTrueForSpecWithoutParent()
	{
		$spec = new Spec();
		$this->assertSame(true, $spec->isRoot());
	...
	public function testIsRoot_ReturnsFalseForSpecWithParent()
	{
		$spec = new Spec();
		$spec->bindParentSpec(new Spec());
		$this->assertSame(false, $spec->isRoot());
...
/**/
	...
	public function testGetRootSpecs_ReturnsAllRootSpecs()
	{
		$specs = $this->createSpecsTree('
			...
			Spec(aaa)
		');
		$this->assertSame(array($specs[1], $specs[0]), $specs['aaa']->getRootSpecs());
		...
		$specs = $this->createSpecsTree('
			...
			Spec(aaa)
		');
		$this->assertSame(array($specs[2], $specs[0]), $specs['aaa']->getRootSpecs());
	}
