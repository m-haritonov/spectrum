	...
	protected function getHtmlForElement($key, $val)
	{
		return
			'<span class="element">' .
				...
				'<span class="key">' . $this->createWidget('code\Operator')->getHtml('[') . htmlspecialchars("$key") . $this->createWidget('code\Operator')->getHtml(']') . '</span>' .
				' ' . $this->createWidget('code\Operator')->getHtml('=>') . ' ' .
				$this->trimNewline($this->createWidget('code\Variable')->getHtml($val, $this->depth + 1)) .
			'</span>';
	}
}